...
class FakeV1Server {
  ...
  Future<void> _handle(HttpRequest req) async {
    ...
    final body = raw.isEmpty ? <String, Object?>{} : jsonDecode(raw);
    ...
    if (path == '/api/v1/sync') {
      final cursor = int.tryParse(req.uri.queryParameters['cursor'] ?? '') ?? -1;
      if (cursor < 0) {
        return _fail(req.response, 409, 'full_sync_required', '游标无效');
      }
      final changes = _changes.where((c) => (c['seq'] as int) > cursor).toList();
    ...
    if (path == '/api/v1/items' && req.method == 'POST') {
      ...
      final item = {
        ...
        'size': utf8.encode('${body['text']}').length,
        'text': body['text'],
        'note': '',
        'pinned': 0,
        'created_at': 1,
        'expires_at': 2,
        'source_device': 'dev-1',
        'target_device': body['target_device'] ?? 'all',
      };
